...
public class XLSXCovertCSVReader {
			...
			if ("v".equals(name)) {
				...
				case NUMBER:
					...
					if (HSSFDateUtil.isADateFormat(this.formatIndex, n)) {
						...
						thisStr = df.format(Double.valueOf(n));
				...
				if (minColumns > 0) {
					...
					if (isCellNull == false && record[0] != null)// 判断是否空行
					{
	...
	public void saveToMysql(String tableName, List<String[]> list) {
		StringBuffer tableStringBuffer = new StringBuffer();
		StringBuffer fieldStringBuffer = new StringBuffer();
		tableStringBuffer.append("CREATE TABLE IF NOT EXISTS " + tableName + " (");
		int row = 1;
		for (String[] record : list) {

			for (String cell : record) {
				if (row == 1) {
					tableStringBuffer.append(cell + "  " + " VARCHAR(255)");
			...
			if (row == 1) {
				tableStringBuffer.append(" ) DEFAULT CHARSET=utf8");
				System.out.println(Db.update(tableStringBuffer.toString()));
			}
